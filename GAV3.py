...
# V3 - Set threshold (only top 90% of fitness is ever selected) instead of random threshold

Threshold = 0.90
MutationPercent = 0.01
InitialPopulationPercent = 0.0001
PartialPopulationReplacement = 0.50
TrainPercent = 0.90
TestPercent = 0.10
Generations = 10
Stats = []
...
def GA(df):
    ...
    for g in range(Generations):
        ...
        for i in range(len(pop)):
            if(random.random() < PartialPopulationReplacement):
...
if __name__ == '__main__':
    ...
    pandas.DataFrame(Stats).to_csv('V3.csv')